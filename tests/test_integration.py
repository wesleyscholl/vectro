...
class TestPerformanceIntegration(unittest.TestCase):
    ...
    def test_memory_efficiency_large_batches(self):
        ...
        results = self.processor.quantize_streaming(self.large_batch, chunk_size=1000, profile="fast")
    ...
    def test_profile_performance_comparison(self):
        ...
        for profile in profiles:
            ...
            results[profile] = {"throughput": throughput, "compression_ratio": compressed.compression_ratio, "elapsed": elapsed}
        ...
        self.assertGreaterEqual(results["quality"]["compression_ratio"], results["fast"]["compression_ratio"])

        print("\nProfile Performance Comparison:")
        for profile, metrics in results.items():
            print(f"  {profile}: {metrics['throughput']:.0f} vec/sec, {metrics['compression_ratio']:.2f}x compression")
...
class TestQualityIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.normalized_vectors = self._normalize_vectors(np.random.randn(200, 384).astype(np.float32))
    ...
    def test_random_vector_quality(self):
        ...
        quality = self.analyzer.evaluate_quality(self.random_vectors, self.vectro.decompress(compressed))
    ...
    def test_sparse_vector_quality(self):
        ...
        quality = self.analyzer.evaluate_quality(self.sparse_vectors, self.vectro.decompress(compressed))
...
class TestEndToEndWorkflow(unittest.TestCase):
    ...
    def test_complete_compression_workflow(self):
        ...
        baseline_stats = {"mean_norm": np.mean(vector_norms), "std_norm": np.std(vector_norms), "vector_count": len(self.test_vectors)}
        ...
        compressed, quality = self.vectro.compress(self.test_vectors, profile=profile_name, return_quality_metrics=True)
    ...
    def test_streaming_workflow(self):
        ...
        for i in range(0, len(large_dataset), chunk_size):
            ...
            decompressed = self.vectro.decompress(compressed)
            cosine_sim = np.mean([np.dot(original, reconstructed) / (np.linalg.norm(original) * np.linalg.norm(reconstructed)) for original, reconstructed in zip(chunk, decompressed)])
    ...
    def test_profile_optimization_workflow(self):
        ...
        optimized_profile = optimizer.auto_optimize_profile(sample_data, target_similarity=0.995, target_compression=3.5)
...
class TestPerformanceRegression(unittest.TestCase):
    ...
    def test_compression_ratio_regression(self):
        ...
        self.assertGreaterEqual(ratio, 3.5, f"Compression ratio regression: {ratio:.2f}× < 3.5× gate")
