...
class VectroQualityAnalyzer:
    ...
    @staticmethod
    def evaluate_quality(original: np.ndarray, reconstructed: np.ndarray, compression_ratio: Optional[float] = None) -> QualityMetrics:
        ...
        cosine_similarities = VectroQualityAnalyzer._compute_cosine_similarities(original, reconstructed)
    ...
    def _compute_vector_ssim(original: np.ndarray, reconstructed: np.ndarray) -> float:
        ...
        covar = np.mean((original - mu_orig[:, np.newaxis]) * (reconstructed - mu_recon[:, np.newaxis]), axis=1)
...
"""
        ...
        sorted_results = sorted(results.items(), key=lambda x: x[1].mean_cosine_similarity, reverse=True)
...
def evaluate_quantization_quality(original: np.ndarray, reconstructed: np.ndarray, compression_ratio: Optional[float] = None) -> QualityMetrics:
