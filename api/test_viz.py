...
from api.main import INDICES, app
...
def _seed(name: str, vectors: list[list[float]]) -> None:
    r = client.post(f"/index/{name}/add", json={"vectors": vectors})
...
def test_project_returns_2d_points_with_required_fields():
    ...
    r = client.post("/index/alpha/project")
...
def test_project_separates_two_well_separated_clusters():
    ...
    body = client.post("/index/blobs/project").json()
...
def test_project_404_for_missing_index():
    r = client.post("/index/does-not-exist/project")
...
def test_cluster_default_k3_returns_assignments_for_every_vector():
    ...
    r = client.post("/index/cl/cluster", json={"k": 3, "max_iter": 20, "seed": 7})
    assert r.status_code == 200
    body = r.json()
...
def test_cluster_custom_k_recovers_three_blobs():
    ...
    body = client.post("/index/three/cluster", json={"k": 3, "seed": 0}).json()
