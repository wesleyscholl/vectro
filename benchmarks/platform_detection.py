...
def get_simd_capabilities() -> List[str]:
    ...
    if os_type == "Linux":
        ...
        if "arm64" in arch:
            ...
            try:
                result = subprocess.run(["sysctl", "-n", "hw.optional.avx512f"], capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip() == "1":
                    capabilities.append("AVX-512F")
                    result = subprocess.run(["sysctl", "-n", "hw.optional.avx512cd"], capture_output=True, text=True, timeout=5)
                    if result.returncode == 0 and result.stdout.strip() == "1":
                        capabilities.append("AVX-512CD")
            except (subprocess.TimeoutExpired, FileNotFoundError):
                pass
            ...
            # Check for AVX2
            try:
                result = subprocess.run(["sysctl", "-n", "hw.optional.avx2_0"], capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip() == "1":
                    capabilities.append("AVX2")
            except (subprocess.TimeoutExpired, FileNotFoundError):
                pass
            ...
            # Check for AVX
            try:
                result = subprocess.run(["sysctl", "-n", "hw.optional.avx1_0"], capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip() == "1":
                    capabilities.append("AVX")
            except (subprocess.TimeoutExpired, FileNotFoundError):
                pass
