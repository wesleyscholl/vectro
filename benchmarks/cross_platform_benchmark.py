...
class CrossPlatformBenchmark:
    ...
    def benchmark_rust_int8_throughput(
        ...
        for dim in dimensions:
            ...
            vectors = np.ascontiguousarray(rng.standard_normal((num_vectors, dim)).astype(np.float32))
    ...
    def print_summary(self):
        ...
        for name, results in by_name.items():
            ...
            for r in sorted(results, key=lambda x: x.dimension):
                print(f"  d={r.dimension:4d}: {r.mean_throughput_vec_per_sec:>12.0f} ± {r.std_throughput_vec_per_sec:>8.0f} vec/s ({r.path_type})")
            print()


def main():
    parser = argparse.ArgumentParser(description="Vectro Cross-Platform Benchmarking Framework")
    parser.add_argument("--output-dir", type=Path, default=None, help="Output directory for results (default: benchmarks/results/cross_platform/)")
    parser.add_argument("--dimensions", type=int, nargs="+", default=[128, 384, 768, 1536], help="Embedding dimensions to benchmark (default: 128 384 768 1536)")
    parser.add_argument("--num-vectors", type=int, default=100000, help="Number of vectors to benchmark (default: 100000)")
    parser.add_argument(
        "--benchmarks",
        type=str,
        nargs="+",
        ...
        help='Which benchmarks to run (default: int8 nf4 binary). Use "rust" to benchmark the vectro_py Rust SIMD path directly.',
    )
    parser.add_argument("--warmup-runs", type=int, default=2, help="Number of warmup runs (default: 2)")
    parser.add_argument("--measurement-runs", type=int, default=5, help="Number of measurement runs (default: 5)")
