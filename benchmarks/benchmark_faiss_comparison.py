...
def benchmark_pq_compression(
    ...
    # Quality
    cosine_sims_vectro = np.sum(test_vectors * vectro_reconstructed, axis=1) / (np.linalg.norm(test_vectors, axis=1) * np.linalg.norm(vectro_reconstructed, axis=1) + 1e-8)
    ...
    if attempt_faiss_import():
        ...
        try:
            ...
            # Quality
            cosine_sims_faiss = np.sum(test_vectors * faiss_reconstructed, axis=1) / (np.linalg.norm(test_vectors, axis=1) * np.linalg.norm(faiss_reconstructed, axis=1) + 1e-8)
...
def benchmark_int8_quantization() -> Dict[str, Any]:
    ...
    if mojo_available:
        ...
        try:
            ...
            start = time.time()
            result = subprocess.run(["./vectro_quantizer", "benchmark", "100000", "768"], capture_output=True, timeout=60, cwd=str(Path(__file__).parent.parent))
            if result.returncode == 0:
                ...
                if vectro_throughput == 0:
                    raise ValueError(f"Could not parse INT8 quantize throughput from output:\n{output}")
                print(f" {vectro_throughput:,.0f} vec/s (Mojo SIMD)")
            else:
                stderr = result.stderr.decode().strip()
                print(f" Error running binary (rc={result.returncode})" + (f": {stderr[:120]}" if stderr else "") + ", falling back to Python")
    ...
    if attempt_faiss_import():
        ...
        try:
            ...
            results["comparison_int8"] = {"vectro_vs_faiss_throughput": round(vectro_throughput / max(faiss_throughput, 0.001), 2), "note": f"Vectro backend: {vectro_backend}"}
        except Exception as e:
            print(f" Error: {e}")
...
def main():
    ...
    all_results = {
        ...
        "notes": {
            "vectro_info": ("Using compiled Mojo SIMD binary" if actual_backend == "Mojo SIMD" else "Mojo binary not found or failed at runtime; results are Python/NumPy"),
