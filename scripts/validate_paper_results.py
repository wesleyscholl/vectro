...
def validate_platform_metadata(platform: Dict) -> Tuple[bool, List[str]]:
    ...
    required_fields = ["os_type", "os_version", "architecture", "cpu_model", "cpu_cores", "cpu_frequency_ghz", "memory_gb", "simd_capabilities", "python_version", "numpy_version", "timestamp"]
...
def validate_int8_throughput(results: List[Dict], cpu_arch: str) -> Tuple[bool, List[str]]:
    ...
    for result in results:
        ...
        if mean_throughput < floor:
            issues.append(f"INT8 d={dim}: {mean_throughput:,.0f} vec/s < {floor:,} floor ({arch_name})")

        # Check CV (statistical stability)
        if cv > VALIDATION_GATES["throughput_cv_percent_max"]:
            issues.append(f"INT8 d={dim}: CV={cv:.1f}% > {VALIDATION_GATES['throughput_cv_percent_max']}% target")
        ...
        if int8_results:
            ...
            with open(csv_file, "w") as f:
                ...
                for r in int8_results:
                    f.write(f"{r['dimension']},{r['mean_vec_per_sec']:.0f},{r['std_vec_per_sec']:.0f},{r['cv_percent']:.1f}\n")
...
Examples:
  ...
  python scripts/validate_paper_results.py --strict
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("benchmarks/results/cross_platform"), help="Directory containing benchmark results JSON files")

    parser.add_argument("--strict", action="store_true", help="Fail on any gate violation (otherwise warnings only)")

    parser.add_argument("--generate-tables", action="store_true", help="Generate LaTeX paper tables")
