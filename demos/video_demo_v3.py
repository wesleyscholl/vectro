...
def main() -> None:
    ...
    row("Quantize throughput", f"{bench['quantize_vecs_per_sec']:,.0f} vec/s", min(bench["quantize_vecs_per_sec"] / 3_000_000, 1.0))
    ...
    row("Compression ratio", f"{ar['compression_ratio']:.2f}×", min(ar["compression_ratio"] / 10, 1.0))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        save_vqz(q_gpu, s_gpu, dims=D, path=vqz, compression="zstd", metadata=b'{"source":"demo","version":"3.0.0"}')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        PQCodebook.load(os.path.join(tmp, "pq.npz"))
