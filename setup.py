    ...
    def run(self):
        ...
        if mojo_source.exists():
            ...
            try:
                ...
                result = subprocess.run(["mojo", "--version"], capture_output=True, text=True, timeout=10)

                if result.returncode == 0:
                    print(f"Found Mojo: {result.stdout.strip()}")

                    # Compile Mojo to binary
                    compile_result = subprocess.run(["mojo", "build", str(mojo_source), "-o", str(mojo_binary)], capture_output=True, text=True, timeout=120)
